...
export const volumeShaders: Record<string, string> = {
 ...
 let tr=max(cov[0][0]+cov[1][1]+cov[2][2],1e-8);cov+=IDENTITY*(tr*.18+1e-7);
 ...
 shapes[i]=Shape(vec4f(centre,rho),vec4f(metric[0],sqrt(cov[0][0])),vec4f(metric[1],sqrt(cov[1][1])),vec4f(metric[2],sqrt(cov[2][2])));
