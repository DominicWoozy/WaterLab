...
import { deferredWaterShading } from './deferred-water-shading.mjs';
...
const optimizedModule = device.createShaderModule({
  code: deferredWaterShading(
    baselineShader(features.includes('float32-filterable')),
  ),
});
const updated = await device.createRenderPipelineAsync({
  layout: device.createPipelineLayout({
    bindGroupLayouts: [renderer.surfaceLayout],
  }),
  vertex: { module: optimizedModule, entryPoint: 'vertex' },
  fragment: {
...
console.log(
  JSON.stringify(
    {
      comparison: 'deferred opaque and transmitted background shading',
      features,
      particles: n,
