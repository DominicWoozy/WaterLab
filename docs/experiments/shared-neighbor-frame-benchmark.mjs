...
const { sharedNeighbors } = await import('./shared-neighbors.mjs');
const { neighborWorkgroups } = await import('./neighbor-workgroups.mjs');
const sharedControl =
  mode === 'shared'
    ? await sharedNeighbors(
        sim,
        Number(process.env.WIDTH || 32),
        process.env.SHARED !== '0',
      )
    : await neighborWorkgroups(sim, Number(process.env.WIDTH || 32));
...
for (let frame = 0; frame < frames; frame++) {
  ...
  sharedControl.enabled = optimized;
...
console.log(
  JSON.stringify(
    {
      mode,
      width: Number(process.env.WIDTH || 32),
      shared: process.env.SHARED !== '0',
      particles: n,
