...
const effect = process.env.EFFECT || 'both';
assert.ok(['reflection', 'shadow', 'both'].includes(effect));
...
for (let frame = 0; frame < frames; frame++) {
  ...
  renderer.encode(
    ...
    h,
    {
      ...
      caustics: true,
      particles: false,
      selfReflection: optimized && effect !== 'shadow',
      selfShadow: optimized && effect !== 'reflection',
    },
  );
...
console.log(
  JSON.stringify(
    {
      comparison: 'optional self optics (on = enabled, off = original)',
      effect,
      features,
      particles: n,
