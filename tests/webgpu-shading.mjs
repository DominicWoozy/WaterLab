// Frozen production images: deferred shading vs the previous eager evaluation.
...
const shader = renderShader(filtered);
// Restore the old evaluation order as the comparison oracle; scene, water,
// receiver geometry and all optical equations remain the production functions.
let eager = shader;
function replace(a, b) {
  assert.ok(eager.includes(a), 'shader order changed');
  eager = eager.replace(a, b);
}
replace(
  'var color=vec3f(0.);',
...
replace(
  ...
  'var background=scene(path.origin,path.direction);if(path.complete==0u){background=sky(path.direction);}',
);
const pipelines = await Promise.all(
  [eager, shader].map(async (code) => {
...
for (const [name, eye, wet] of [
  ...
  const a = await probe(pipelines[0]),
    b = await probe(pipelines[1]);
  let max = 0,
    squared = 0;
  ...
  assert.ok(max < 1e-5, `${name}: image changed by ${max}`);
  results.push({ name, max, rms: Math.sqrt(squared / a.length) });
}
assert.deepEqual(errors, []);
console.log(JSON.stringify({ filtered, results, errors }));
console.log(
  'PASS: water, airborne layers/drop, duck occlusion and dry background retain their image',
);
process.exit(0);
